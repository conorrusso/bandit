...
class TestRenewalDateValidation:
    """Renewal date format: MM/YYYY — validated inline in _q12_renewal."""

    def test_valid_format_accepted(self):
        """MM/YYYY is valid."""
        import re
        raw = "06/2027"
        matched = bool(re.match(r"^\d{2}/\d{4}$", raw))
        assert matched is True

    def test_invalid_format_rejected(self):
        """YYYY-MM (ISO) does not match the MM/YYYY pattern."""
        import re
        raw = "2027-06"
        matched = bool(re.match(r"^\d{2}/\d{4}$", raw))
        assert matched is False

    def test_plain_year_rejected(self):
        import re
        assert not re.match(r"^\d{2}/\d{4}$", "2027")

    def test_correct_format_month_first(self):
        import re
        assert re.match(r"^\d{2}/\d{4}$", "12/2028")


class TestItActionsGenerated:
    """IT actions are generated by IntakeWizard.build_it_actions()."""

    def _make_wizard_with_answers(self, answers: dict):
        """Return a wizard instance with pre-filled answers, bypassing interactive prompts."""
        from unittest.mock import MagicMock
        from core.profiles.intake import IntakeWizard
        wizard = IntakeWizard.__new__(IntakeWizard)
        wizard.vendor_name = "TestVendor"
        wizard.answers = answers
        wizard.config = MagicMock()
        wizard.config.get_idp_name.return_value = "Okta"
        wizard.console = MagicMock()
        return wizard

    def test_identity_integration_generates_sso_actions(self):
        """identity_access category → SSO provisioning actions in output."""
        wizard = self._make_wizard_with_answers({
            "integrations": [
                {
        ...
        actions = wizard.build_it_actions()
        assert len(actions) > 0
        actions_text = " ".join(actions)
        assert "Okta" in actions_text

    def test_no_integrations_returns_empty_list(self):
        wizard = self._make_wizard_with_answers({"integrations": [], "sso_required": False})
        actions = wizard.build_it_actions()
        assert actions == []

    def test_sso_required_adds_sso_action(self):
        """sso_required=True adds SSO setup action at the front."""
        wizard = self._make_wizard_with_answers({
            "integrations": [],
            "sso_required": True,
        })
        ...
        assert "SSO" in actions[0]


class TestDataTypeConsistency:
    """_check_data_type_consistency detects integration/data-type mismatches.

    This method is interactive (uses Prompt.ask) so we verify the detection
    logic by inspecting the mismatches it would generate, not by running it.
    """

    def test_healthcare_integration_without_phi_is_mismatch(self):
        """healthcare_clinical integration without phi in data_types is a mismatch."""
        answers = {
            "data_types": ["customer_pii"],
            "integrations": [
                {
        ...
        # Build the mismatch detection logic inline (mirrors _check_data_type_consistency)
        stated_types = set(answers["data_types"])
        mismatches = []
        for integration in answers["integrations"]:
            ...
            if (cat == "healthcare_clinical"
                ...
                mismatches.append("phi")
        assert "phi" in mismatches

    def test_no_mismatch_when_phi_already_stated(self):
        """If phi is in data_types, no mismatch is detected for healthcare integration."""
        answers = {
            "data_types": ["phi"],
            "integrations": [
                {"category": "healthcare_clinical", "system_name": "Epic"}
            ],
        }
        stated_types = set(answers["data_types"])
        mismatches = []
